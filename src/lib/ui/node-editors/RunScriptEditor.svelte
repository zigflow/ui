...
<script lang="ts">
  ...
  const config = $derived(taskNode.config as RunScriptConfig);
  ...
  function emit(updates: Partial<Omit<RunScriptConfig, 'kind'>>): void {
...
<div class="node-editor">
  <h3 class="editor-title">{t('inspector.run.script.title')}</h3>

  <!-- Language -->
  <div class="field-row">
    <label class="field-label" for="rs-language">
      {t('inspector.run.script.language.label')}
    </label>
    <select
      id="rs-language"
      class="field-select"
      aria-label={t('inspector.run.script.language.label')}
  ...
  <!-- Code -->
  <div class="field-row">
    <label class="field-label" for="rs-code">
      {t('inspector.run.script.code.label')}
    </label>
    <textarea
      id="rs-code"
      class="code-textarea"
      class:code-textarea--error={codeError}
      aria-label={t('inspector.run.script.code.label')}
      value={config.code}
      oninput={(e) => handleCodeChange(e.currentTarget.value)}
    ></textarea>
    {#if codeError}
      <p class="field-error" role="alert">
        {t('inspector.run.script.code.required')}
      </p>
    {/if}
  </div>
  ...
  <div class="section-group">
    ...
    <ArgumentsList
      {args}
      addLabel={t('inspector.run.script.arguments.addRow')}
  ...
  <div class="section-group">
    <h4 class="section-subtitle">{t('inspector.run.script.env.title')}</h4>
          ...
          <li class="assignment-item">
            <div class="assignment-row assignment-row--key">
              <input
                ...
                type="text"
                aria-label={t('inspector.run.script.env.keyLabel')}
                value={key}
                oninput={(e) =>
              ...
              <button
                class="remove-btn"
                type="button"
                aria-label={t('inspector.run.script.env.removeRow')}
            ...
            <div class="assignment-row assignment-row--value">
              <input
                ...
                type="text"
                aria-label={t('inspector.run.script.env.valueLabel')}
              ...
              <select
                ...
                aria-label={t('inspector.run.script.env.overrideLabel', {
                  key,
                })}
                value={override}
                onchange={(e) =>
    ...
    <button class="add-btn" type="button" onclick={handleEnvAdd}>
      {t('inspector.run.script.env.addRow')}
    </button>
  </div>
</div>
